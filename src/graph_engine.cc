...
std::string
GraphEngine::PostGraphRequest(graph::Request& request) {
  ...
  // Build Graph
  GraphSharedPtr graph_shared_ptr = std::make_shared<Graph>(num_nodes, adj_list);
...
std::string
GraphEngine::DeleteGraphRequest(graph::Request& request) {
...
std::string
GraphEngine::MinDistanceGraphRequest(graph::Request& request) {
  ...
  // Lock the graph db to check for the graph
  {
    ...
    if (it != graph_db.end()) {
      ...
      return "OK: min dist is "+ std::to_string(min_dist);
    } else {
      return "ERROR: Graph node present in DB";
    }
  }
}
